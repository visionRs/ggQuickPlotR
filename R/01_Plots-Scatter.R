...
scatter_plot <- function(data=dt,
  ...
  code <-  HTML(paste0('<pre>ggplot(data = ',df_name, ' , aes(x=', x, ', y=', y, ifelse(colorby=="None" & shapeby == 'None',")) + <br> ",ifelse(colorby == 'None' & shapeby != 'None', paste0(',' ,'shape = ',shapeby, ')) + <br>'),
                              ...
                              ifelse(regressionLine == TRUE  & correlation == TRUE, paste0("geom_smooth(method=lm, se=FALSE) + stat_cor(method = 'pearson') + <br>"),' + <br>'),
                              ifelse(regressionLine == TRUE & correlation == "NULL", paste0("geom_smooth(method=lm, se=FALSE) + <br>"),' + <br>'),
